...
(function ($) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        let modeButton = document.getElementById("mode-setting-btn");
        let radios = document.querySelectorAll("input[type=radio]");
    
        function updateTheme(newTheme) {
            Object.keys(newTheme).forEach(key => {
                document.body.setAttribute(`data-${key}`, newTheme[key]);
            });
        ...
        if (modeButton) {
            modeButton.addEventListener("click", function () {
                ...
                let isDarkMode = body.getAttribute("data-layout-mode") === "dark";
                let newMode = isDarkMode ? "light" : "dark";
                let newSidebar = isDarkMode ? "light" : "dark";
                let newTopbar = isDarkMode ? "light" : "dark";
    
                body.setAttribute("data-layout-mode", newMode);
                body.setAttribute("data-sidebar", newSidebar);
                body.setAttribute("data-topbar", newTopbar);
    
                localStorage.setItem("theme", JSON.stringify({
                    "layout-mode": newMode,
                    "sidebar-color": newSidebar,
                    "topbar-color": newTopbar
                }));
    
                let moonIcon = modeButton.querySelector(".layout-mode-dark");
                let sunIcon = modeButton.querySelector(".layout-mode-light");
        ...
        fetch("/save-theme")
            ...
            .then(data => {
                for (const key in data) {
                    ...
                    document.body.setAttribute(`data-${key}`, data[key]);
                }
            })
